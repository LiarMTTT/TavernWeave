...
TavernWeave 是面向 Codex 与 Claude Code、并为 DeepSeek Harness 提供实验适配的 SillyTavern 制卡与 Vibe Coding 工程系统：21 个可路由 Skill、从模糊愿望到总设计案/首版/蓝图的项目编排、可再次核验且不可改写的本地成长履历、一份可恢复的创作权威、一条从源码到真实宿主和人工验收的证据链、统一随包的 ST/设计/动效资料库，以及可单席或三席联动的 Soul 模式。

当前正式版为 **[v1.4.0](https://github.com/LiarMTTT/TavernWeave/releases/tag/v1.4.0)**，包含 21 个原生 Skill：新增“文本精修 · 去 AI 味与去八股”，支持单次、任务持续、经同意的工作区／客户端持久规则；审查增加全量、架构和全量架构入口，覆盖软件、角色卡和混合项目。奶人教程同步完成全文精修，并提供驾驶员在场、无人值守和返回在场的可复制提示词。
...
完成 21/21 核验后新建任务或重启会话，使宿主重新发现 Skill。新任务中仍需实际调用 Soul、脑暴编排、照镜子与 Library；安装回执不能冒充宿主发现。v1.4.0 的完整 ZIP 与 SHA-256 校验文件可从 [正式发布页](https://github.com/LiarMTTT/TavernWeave/releases/tag/v1.4.0) 下载。
