...
当前正式版本为 **[v1.3.0](https://github.com/LiarMTTT/TavernWeave/releases/tag/v1.3.0)**。本版新增第 20 个原生 Skill“照镜子 · Vibe Code 成长历程”：从当前可用证据重新评估完整能力网络，输出高密度七章交互报告与可独立保存的文字评估表单，并以脱敏、不可改写、可验证哈希链保存本地履历。源码、发布资产、实际安装、宿主发现、真实运行与驾驶员验收仍是彼此独立的证据门，不能互相代替。
