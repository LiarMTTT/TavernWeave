...
description: Route TavernWeave work to the smallest relevant set of bundled ST development guides, the standing A0 engineering checklist, the frontend design catalog, motion catalog, linked source Wiki pages, and the offline picker. Use before a TavernWeave write task, when the user asks how to build or choose a SillyTavern feature, when another TavernWeave skill needs domain guidance or design references, or when the user says the agent keeps ignoring the guide database. Also use to inspect source, license, snapshot, and selection receipts. Do not load the whole library into context, publish excluded driver material, or treat a catalog candidate as an adopted implementation.
---
...
4. Read only the returned guide and Wiki paths. Use `--include-experimental` only when the user is explicitly evaluating an experimental route such as C8.
5. Return a short receipt containing route ID, loaded document IDs, snapshot version, candidate selections, and unresolved source/runtime checks.
...
- `design` and `motion` records are candidates. “Add to selection” means proposed, not adopted, installed, licensed for every use, or implemented.
...
Open `assets/picker/index.html` in a browser when visual comparison is useful. It supports ST guides, design, motion, Wiki/source links, candidate selection, copied JSON, keyboard navigation, narrow screens, and reduced motion.
...
- Archived command-era B1, process records, local evidence, raw Vault material, inboxes, private RAG, logs, and unrelated AFV knowledge domains are not distributable through this skill.
...
The snapshot command copies 32 formal guides plus A0, 82 design records, 38 motion records, 18 directly linked Wiki pages, and owned preview sandboxes. It produces portable hashes and refuses excluded filenames, private absolute paths, missing sources, count drift, and untracked snapshot files. AFV Git or Release state is provenance metadata, not a prerequisite for this content snapshot.
