...
- 新增第 22 个 Skill：作品资料库。小说、原作设定、同人材料、联网文字和实际看过的图片参考可按统一来源链整理，支持按人物或专题逐步建设。
- 默认交付可人工编辑的 Markdown 和可重建的 JSONL 索引，按需导出 SQLite；按原作/同人、改编版本和知情阶段检索，给二创提供可回查出处的资料包。
- 长任务先评估代表样本和实际/估算/不可见 token，再按批次和调用预算接续；保留失败重试用量、人工修改冲突、中断恢复和唯一落库者。
- 联网与看图使用实际宿主能力。图片默认保存参考信息与链接，后台执行取决于宿主；本地工具不代替模型精读或自动判断事实正确。
- 本地工具与隔离安装检查通过；真实作品 token 对照、宿主重新发现与驾驶员体验保留独立检查。升级清单为 22 个 Skill，实际客户端安装由用户安排。

## 1.5.0 - 2026-09-07

- 全部 21 个 Skill 与全局入口接入持续大白话、必要术语的上下文对照，以及所有挡位的必要/故障详细解释。
- 增加新人、入门、熟练、老手四档用户选择；独立偏好区块支持查询、预览和明确授权后的保存，保留升级与模式退出后的选择，不依据表现自动切档。
- 增加接手角色卡二创/续更入口，串联看懂原卡、保留项、首次修改、试用与再改一次；普通软件和首次体验使用同一引导基线。
- 更新教程与安装说明；文件匹配和实际客户端加载分别报告。Codex 有效全局文件解析支持非空 override，Claude 配置目录跟随其环境配置。
- 修正真实 Codex 安装中发现的两项兼容问题：识别基础版本一致的官方本地缓存后缀，仍严格校验版本与 Skill 内容；插件首页保留三个推荐开工入口，避免超出客户端上限后整组被忽略。
- 修正中文 Windows 默认控制台编码下的安装检查：挡位管理器使用标准 JSON 转义传递中文，避免旧代码页破坏结果；解析后的说明、挡位及路径保持原文。
- 持久偏好、前门升级与安装行为以隔离测试验证；真实客户端安装/加载、酒馆试用、驾驶员验收与发布继续单独留证。

## 1.4.0 - 2026-09-04

### 文本精修与模式管理

- 新增第 21 个原生 Skill“文本精修 · 去 AI 味与去八股”，提供轻润色、结构改写和深度重写，保留原意、人物声音、事实及技术结构。
- 支持单次、当前任务持续，以及分别经驾驶员同意的工作区／客户端持久模式；可查看状态、临时退出或移除指定持久规则。
- 持久规则管理器随 Skill 分发，支持实际规则文件预览、独立区块、变更检查、备份、幂等启停及文件回执。

### 全量与架构审查

- 新增全量审查、架构审查、全量架构审查入口，覆盖软件工程、角色卡和混合项目。
- 报告合并器兼容旧格式，保留覆盖缺口、排除理由、同路径冲突及全部发现；灵魂杀手的三条上限只作用于全量／架构报告摘要。
- 同步 21 项安装清单、Host Front Door、Library 路由、DSH 离线目录与教程。DSH 持久规则和真实宿主兼容性仍不作新增承诺。

### 奶人教程全文精修

- 精修功能地图与 00–45 教程正文，融入作者的见解与调侃，用具体操作讲清概念、Agent 分工，以及需要驾驶员验收的面与点。
- 新增驾驶员在场、无人值守和返回在场三组可复制提示词，延续“AI 高达、Skill＋Harness 操作系统、驾驶员”的教学视角。
- 补清 MVU 更新、Zod 结构校验、模型判断、脚本计算与运行调试的关系；同步目录和选型文案，保留技术代码、已确认样章与历史发布记录。
