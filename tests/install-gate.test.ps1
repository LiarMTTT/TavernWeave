...
try {
    ...
    Write-Output 'Install gate tests passed: legacy gap rejected, 21/21 upgrade passed, clean install passed, Host Front Door install/receipt/idempotence passed, drift rejected, picker loss rejected, unsafe target rejected, source-tree impersonation rejected, linked-target rollback passed.'
} finally {
